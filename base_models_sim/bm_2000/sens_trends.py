"""
This file is to calcualte the descriptors based on the species ratio profile data from Cantera simulation
The descriptors will be used in sensitivity calculation.
    ...
    for f in f_paths:
        if os.path.exists(f):
            ...
            if os.path.exists(file_path):
                ...
                sens_data = [[rxn_id, ratio] + [0] * 15]
                for i in range(1, len(output)):
                    sens = calculate(output_data, i, type='sens')
                    sens.insert(0, ratio)
                    sens.insert(0, rxn_id)
                    sens = np.array(sens)
                    sens[np.isinf(sens)] = 1e-15
                    sens = np.nan_to_num(sens)
                    sens_data.append(sens)
                ...
                df = pd.DataFrame(np.array(sens_data), columns=table_names)
                output_dir = Path(f"sens_trends/{f.split('/')[1]}/{f.split('/')[2]}/kin_sens_trend_{rxn_id}.csv")
