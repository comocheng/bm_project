...
def calculate(data, sens_id, type='sens'):
    """
    Calculate properties of interest from the raw data
    :param data: the data
    :param sens_id: the reactor id to calculate the sensitivity
...
if __name__ == "__main__":        
    ...
    for i in range(len(rtols)):
        for ratio in [.6, 1., 1.1, 1.2, 1.6, 2., 2.6]:
            f_paths.append(f'kinetic_sens/rtol_{rtols[i]}_atol_{atols[i]}/{ratio}')
    
    # read from reference file to extract the half temperature point at each ratio for sensitivity calculation 
    ref_data = pd.read_csv('sim_data/therm_sens_ref.csv')
    pos = ref_data.loc[:,'Half Temp Pos'].astype('int')
    rat = ref_data.loc[:,'Ratio'].round(2)
    half_temp_idice = dict(zip(rat, pos))
            
    for f in f_paths:
        data_to_csv = []
        if os.path.exists(f):
            half_temp_id = half_temp_idice[float(os.path.split(f)[-1])]
            for rxn_id in range(107):
                file_path = os.path.join(f, f'kin_sens_{rxn_id}.csv')
                if os.path.exists(file_path):
                    ...
                    if len(output) >= 7000:
                        ...
                        sens_data = calculate(output_data, half_temp_id, type='sens')
                        sens_data.insert(0, rxn_id)
            ...
            df.to_csv(os.path.join(f, 'kin_sens_properties.csv'))
