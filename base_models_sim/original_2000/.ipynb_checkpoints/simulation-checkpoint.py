...
def calculate(data, type='sens'):
    ...
    #     if x[0] == 'CH4(2)':
    #         if len(x[1][0]) > 2000:
    #             value_sens_pos = (x[1][0][2000] - x[1][0][1000]) / 2 +  x[1][0][1000]
    #             diff_arr = np.absolute(np.array(x[1][0]) - value_sens_pos)
    #             sens_id = diff_arr.argmin() 
    #         else:
    #             sens_id = -1
    ...
    if type is 'sens':
        ...
        return ratio, ch4_in, ch4_out, co_out, h2_out, h2o_out, co2_out, reference_exit_temp, reference_peak_temp, reference_peak_temp_dist, reference_o2_conv, reference_max_ch4_conv, reference_dist_to_50_ch4_conv, sens_id
...
if __name__ == "__main__":
    ...
    for tols in tol_comb:
        ...
        data = pool.map(partial(run_one_simulation, 'base_modified_pt.yaml', tols[0], tols[1]), ratios, 1) #use functools partial
        pool.close()
        pool.join()
        # try:
        ...
        k.columns = ['C/O ratio', 'CH4 in', 'CH4 out', 'CO out', 'H2 out', 'H2O out', 'CO2 out', 'Exit temp', 'Max temp', 'Dist to max temp', 'O2 conv', 'Max CH4 Conv', 'Dist to 50 CH4 Conv', 'Half Temp Pos']
        ...
        plot_ratio_comparisions('base_modified_pt.yaml', ratio_comparison)
