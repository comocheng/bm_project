...
def run_one_simulation(path_to_cti, rtol, atol, ratio):
    ...
    try:
        ...
        tol_path = f'base_data/rtol_{rtol}_atol_{atol}'
        if not os.path.isdir(tol_path):
            os.makedirs(tol_path)
        data_path = os.path.join(tol_path, f'base_{ratio}.csv')
...
if __name__ == "__main__":
    ...
    sols = setup_ct_solution('base_modified_pt.yaml')
    for tols in tol_comb:
        ratios = [.6, 1., 1.1, 1.2, 1.6, 2., 2.6]
        data = []
        num_threads = min(multiprocessing.cpu_count(), len(ratios))
        pool = multiprocessing.Pool(processes=num_threads)
        pool.map(partial(run_one_simulation, 'base_modified_pt.yaml', tols[0], tols[1]), ratios, 1)
        pool.close()
        pool.join()
