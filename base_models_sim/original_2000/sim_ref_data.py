...
if __name__ == "__main__":        
    ...
    for i in range(len(rtols)):
        f_paths.append(f'base_data/rtol_{rtols[i]}_atol_{atols[i]}')

    for f in f_paths:
        if os.path.exists(f):
            for ratio in ratios:
                file_path = os.path.join(f, f'base_{ratio}.csv')
                if os.path.exists(file_path):
                    ...
                    df.to_csv(os.path.join(f, f'ref_{ratio}.csv'), index=False)
